...
hotel_list = []
line_data1 = csv.reader(open("3star酒店信息.csv", 'r', encoding='utf-8'))  # 读取原始文件数据
...
for i in range(0, len(hotel_list)):
    num = 0
    sum1 = 0
    line_data2 = csv.reader(open("3star_sort_line_sum.csv", 'r', encoding='utf-8'))  # 读取原始文件数据
    ...
    if num == 9:
        ...
        n += 1
        print(n)
        with open("3starresult.csv", 'a', newline='', encoding='utf-8') as d:  # numline是来控制空的行数的
